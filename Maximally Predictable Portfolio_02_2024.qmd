...
Equal_Weighted_cumulative<- as.matrix(cumprod(Equal_Weighted+1))
...
```{r}
BTC<-cumprod((ret[-c(1:window),])$Close_BTC+1)
```
...
```{r}
out_of_sample<-data.frame(cumprod(returns1+1))
...
```{r}
sum(cumprod(returns+1))
```

```{r}
summary(cumprod(returns+1))
```
